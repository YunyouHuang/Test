...
def get_dataset_array_by_id(X,y,k):
    ...
    for i in range(0,k):
        ...
        for j in range(0,len(data)):
            if (int(data[j][0]) in slice_user_id_1) or (int(data[j][0]) in slice_user_id_0):
                tmp_indexs.append(j)
        index_array.append(np.array(tmp_indexs))
    return index_array

def mknfold(dall, nfold, param, seed, evals=(), fpreproc=None, stratified=False,
            folds=None, shuffle=True, X=None, y=None):
    """
    ...
    if stratified is False and folds is None:
        ...
        idset=get_dataset_array_by_id(X,y,nfold)
...
def cv(params, dtrain, num_boost_round=10, nfold=3, stratified=False, folds=None,
       ...
       seed=0, callbacks=None, shuffle=True, X=None, y=None):
    # pylint: disable = invalid-name
